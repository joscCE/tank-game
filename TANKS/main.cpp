...
const float matchDuration = 300.0f; // Duración de la partida en segundos
...
void CountTanksByColor(Tank* player1Tanks, Tank* player2Tanks, int& yellowCount, int& skyBlueCount, int& redCount, int& blueCount) {
    ...
    // Contar tanques activos de cada color en el Jugador 2
    for (int i = 0; i < numTanksPerPlayer; i++) {
        if (player2Tanks[i].active) {
            if (ColorsAreEqual(player2Tanks[i].color, RED)) redCount++;
            else if (ColorsAreEqual(player2Tanks[i].color, BLUE)) blueCount++;
        }
    }
}

// Nueva función para verificar colisión entre bala y tanque
